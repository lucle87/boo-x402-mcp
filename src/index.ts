...
import { TOOLS, resolveRoute, displayPrice, type Tool } from "./lib/services.js";
...
function toZodShape(tool: Tool): ZodRawShape {
  const shape: ZodRawShape = {};
  for (const [name, spec] of Object.entries(tool.input)) {
    let base: z.ZodTypeAny;
    if (spec.type === "string") {
      base = spec.enum && spec.enum.length ? z.enum(spec.enum as [string, ...string[]]) : z.string();
