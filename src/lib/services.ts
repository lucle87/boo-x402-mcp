...
/** Mot dich vu backend cu the ma tool goi toi. */
export type Route = {
  url: string;                                   // URL day du
  priceUsd: number;
  /** Bien doi args cua tool -> body that su cua endpoint. */
  body: (a: Record<string, any>) => Record<string, any>;
};
...
export type Tool = {
  ...
  /** Hoac 1 route co dinh... */
  route?: Route;
  /** ...hoac dinh tuyen theo gia tri cua truong `routeBy` (thuong la `chain`). */
  routeBy?: string;
...
export function resolveRoute(tool: Tool, args: Record<string, any>): Route {
  if (tool.route) return tool.route;
...
export function displayPrice(tool: Tool): number {
  if (tool.route) return tool.route.priceUsd;
  if (tool.routes) return Math.max(...Object.values(tool.routes).map((r) => r.priceUsd));
  return 0;
}
