...
function getPayFetch() {
    ...
    if (!pk) {
        throw new Error("WALLET_PRIVATE_KEY is not set. These tools call paid APIs (USDC on Base via x402). " +
    ...
    const client = new x402Client()
        ...
        .registerPolicy((_version, reqs) => reqs.filter((r) => {
 ...
 */
export async function callPaid(url, body, priceUsd) {
    if (priceUsd > MAX_USD_PER_CALL) {
        throw new Error(`This call costs $${priceUsd} which exceeds MAX_USD_PER_CALL ($${MAX_USD_PER_CALL}). ` +
            `Raise MAX_USD_PER_CALL in the MCP config if you want to allow it.`);
    }
    if (spentUsd + priceUsd > MAX_USD_TOTAL) {
        throw new Error(`Session spend cap reached: already spent $${spentUsd.toFixed(4)} of MAX_USD_TOTAL ($${MAX_USD_TOTAL}). ` +
